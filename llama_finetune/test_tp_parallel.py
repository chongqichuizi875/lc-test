...
for name, module in model.named_modules():
    if name.endswith("mlp"):
        ...
        print(f"ori mlp {dist.get_rank()}: {ori_mlp.gate_proj.weight}")
        print(f"ori split {dist.get_rank()}: {nn.Parameter(torch.split(ori_mlp.gate_proj.weight, ori_mlp.gate_proj.out_features // tp, dim=0)[dist.get_rank()])}")
        print(f"mlp {dist.get_rank()}: {module.down_proj.weight}")
        break
